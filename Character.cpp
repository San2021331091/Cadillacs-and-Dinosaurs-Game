...
void Character :: enemy_calculate_hit_boxes(enemy_t* enemy) {
    ...
    if (type == ENEMY_TYPE_BUTCHER) {
        box->left = (long)(center_x - (50 * sprite->scale));
        box->top = (long)((pos->y + pos->z) - (120 * sprite->scale));

        box->right = (long)(center_x + (50 * sprite->scale));
        box->bottom = (long)(box->top + scaled_height);
    } else {
        box->left = (long) (pos->x + (scaled_width/4));
        box->top = (long)(pos->y + pos->z - scaled_height);
    ...
    if (state == CHARACTER_STATE_IDLE ||
        ...
        sprite_t* damage_sprite = damagefx->sprite;
...
void Character::player_calculate_hit_boxes(player_t* player) {
    character_t* character = &player->base;
    RECT* box = nullptr;
