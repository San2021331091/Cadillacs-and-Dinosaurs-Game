...
player_t* Player :: player_new() {
    ...
    player->base.sprite = Sprite_Handle::sprite_new(L"E:/ClionProjects/Cadillacs_and_Dinosaurs_Game/Images/player-jack.bmp", 120, 110, 2.5);
    ...
    Animation_Handle::sprite_add_animation(player->base.sprite, L"idle", 0, 0, 0);
    a = Animation_Handle::sprite_add_animation(player->base.sprite, L"walk", 1, 12, 0);
    a->speed = 1.2f;
    a = Animation_Handle::sprite_add_animation(player->base.sprite, L"punch", 13, 15, 1);
    a->speed = 3.5f;
    a->play_once = 1;

    player->anim_upper_cut = Animation_Handle::sprite_add_animation(player->base.sprite, L"punch2", 18, 21, 1);
    player->anim_upper_cut->speed = 1.5f;
    player->anim_upper_cut->play_once = 1;

    Animation_Handle::sprite_add_animation(player->base.sprite, L"hit", 16, 16, 0);

    a = Animation_Handle::sprite_add_animation(player->base.sprite, L"fall", 30, 35, 0);
    a->speed = 0.5f;
    a->play_once = 1;

    a = Animation_Handle::sprite_add_animation(player->base.sprite, L"jump", 24, 26, 1);
    a->speed = 1.3f;
    a->play_once = 1;

    a = Animation_Handle::sprite_add_animation(player->base.sprite, L"win", 27, 29, 0);
    a->speed = 0.5f;
    a->play_once = 1;
...
void  Player :: player_set_state(player_t* player, int state) {
    ...
    switch(state) {
        case CHARACTER_STATE_IDLE:
           Animation_Handle:: sprite_set_animation(player->base.sprite, L"idle");
            break;
        case CHARACTER_STATE_WALKING:
            Animation_Handle::sprite_set_animation(player->base.sprite, L"walk");
            break;
        case CHARACTER_STATE_JUMP:
            Animation_Handle::sprite_set_animation(player->base.sprite, L"jump");
            if (ch->on_ground) {
                ch->velocity.y = -18.0f;
            }
            break;
        case CHARACTER_STATE_FIGHT:
            ...
            if (player->hit_streak >= 2) {
                Animation_Handle::sprite_set_animation(player->base.sprite, L"punch2");
            } else {
                Animation_Handle::sprite_set_animation(player->base.sprite, L"punch");
            }
            break;
        case CHARACTER_STATE_HIT:
            Animation_Handle::sprite_set_animation(player->base.sprite, L"hit");
            if (player->base.state != state) {
                player->hit_recover_time = 10;
            }
            break;
        case CHARACTER_STATE_HIT_FALL:
            Animation_Handle::sprite_set_animation(player->base.sprite, L"fall");
            if (player->base.state != state) {
                player->hit_recover_time = 300;
            }
            break;
        case CHARACTER_STATE_WIN:
            Animation_Handle::sprite_set_animation(player->base.sprite, L"win");
            break;
    }
    if (player->base.state != state) {
